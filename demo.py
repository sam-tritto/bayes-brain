...
import sqlite3
import random
from typing import Sequence, List, Tuple, Dict, Any, Union, Optional

try:
    import numpy as np
    from scipy.stats import beta as scipy_beta
except ImportError:
    print("⚠️  This demo requires 'numpy' and 'scipy' to perform math and render sparklines.")
...
class DemoEmbedder:
    ...
    def embed_query(self, text: str) -> Sequence[float]:
        ...
        if any(w in text_lower for w in ["sql", "database", "query", "select", "table", "postgres", "sqlite", "insert", "db"]):
            v[0] = 1.0
        # Coding/Python-related keywords
        if any(w in text_lower for w in ["python", "code", "script", "function", "asyncio", "loop", "regex", "algorithm"]):
            v[1] = 1.0
        # Web Search/General keywords
        if any(w in text_lower for w in ["search", "find", "news", "weather", "latest", "stock", "google", "policy", "wiki"]):
            v[2] = 1.0
...
def register_context_label(router: BayesianRouter, prompt: str):
    ...
    if ctx_key not in context_labels:
        ...
        elif any(w in text_lower for w in ["python", "code", "script", "function", "asyncio", "loop", "regex", "algorithm"]):
            context_labels[ctx_key] = "Python Programming Tasks"
        elif any(w in text_lower for w in ["search", "find", "news", "weather", "latest", "stock", "google", "policy", "wiki"]):
...
def retrieve_all_beliefs(router: BayesianRouter) -> Dict[str, Dict[str, Dict[str, float]]]:
...
def print_beliefs_dashboard(router: BayesianRouter):
    ...
    if not beliefs:
        print("   No active beliefs recorded in storage yet. Try routing some tasks first!")
        print("=" * 90 + "\n")
        return
    ...
    for ctx_key, candidates_params in beliefs.items():
        ...
        print("-" * 90)
        print(f"{'Candidate / Tool':<20} | {'Alpha (Succ)':<12} | {'Beta (Fail)':<11} | {'Expected Rate':<13} | {'Posterior Sparkline (0 to 1)':<30}")
        print("-" * 90)
        for candidate_name in CANDIDATES:
            ...
            print(f"{candidate_name:<20} | {alpha:<12.2f} | {beta:<11.2f} | {expected_rate:>11.1f}% | `\033[32m{sparkline}\033[0m`")
    print("=" * 90 + "\n")
...
def run_interactive_loop(router: BayesianRouter):
    ...
    while True:
        ...
        print("======================================================================")
        print("Enter prompts and guide the multi-armed bandit manually by marking decisions")
        ...
        if feedback_choice in ["y", "yes"]:
            router.feedback(context_text=prompt, candidate_name=chosen_tool, success=True)
            print("\033[32m✔ Logged SUCCESS feedback. Alpha incremented.\033[0m")
        elif feedback_choice in ["n", "no"]:
            router.feedback(context_text=prompt, candidate_name=chosen_tool, success=False)
...
def run_automated_simulation(router: BayesianRouter, rounds: int = 100):
    ...
    successes = {t: 0 for t in CANDIDATES}
    ...
    for i in range(1, rounds + 1):
        ...
        router.feedback(context_text=prompt, candidate_name=chosen_tool, success=success)
        ...
        print(f"Round {i:03d}/{rounds:03d} | Category: {category:<6} | Routed to: {chosen_tool:<15} | Result: {result_str}")
        time.sleep(0.03)
    ...
    for tool in CANDIDATES:
        ...
        print(f" - {tool:<15}: Selected {cnt:02d} times | Actual Success: {succ:02d}/{cnt:02d} ({rate:.1f}%)")
    print("-" * 50)
...
def run_drift_simulation():
    ...
    print(" - Rounds 1 to 25:   sql_expert has a 95% success rate; python_expert is 50%.")
    print(" - Rounds 26 to 50:  sql_expert suffers database downtime, success drops to 10%!")
    ...
    for i in range(1, 51):
        ...
        success = random.random() <= success_prob
        drift_router.feedback(context_text=prompt, candidate_name=chosen_tool, success=success)
        ...
        result_str = "\033[32mSUCCESS\033[0m" if success else "\033[31mFAILURE\033[0m"
        print(f"Round {i:02d}/50 | {phase_str:<32} | Routed: {chosen_tool:<15} | Outcome: {result_str} | sql: (α={sql_p['alpha']:.1f}, β={sql_p['beta']:.1f}) | python: (α={py_p['alpha']:.1f}, β={py_p['beta']:.1f})")
        time.sleep(0.1)
        
    print("\nDrift simulation complete!")
    print("\nDid you notice?")
    print(" - In Rounds 1-25, the router converged on sql_expert.")
    print(" - When sql_expert started failing (Rounds 26+), the old success parameter (Alpha)")
    print("   was decayed exponentially by 0.90 every round, making its parameter values drop.")
    print(" - Within a few rounds, the router successfully shifted traffic to python_expert!")
...
def main():
    ...
    while True:
        ...
        print("-" * 70)
        print("1) 🛠️  Manual Interactive Loop (enter prompts & evaluate success manually)")
        print("2) ⚡ Automated Convergence Simulation (runs 100 rounds with preset rates)")
        print("3) 🌊 Non-Stationary Drift Simulation (shows how decay adapts when tools degrade)")
